...
- (void)registerCell {
    [self.collectionView registerNib:[UINib nibWithNibName:CARD_ROW_ITEM
                                                    bundle:nil]
          forCellWithReuseIdentifier:CARD_ROW_ITEM];
    
    [self.collectionView registerNib:[UINib nibWithNibName:CARD_TITLE_ITEM
                                                    bundle:nil]
          forCellWithReuseIdentifier:CARD_TITLE_ITEM];
    
    [self.collectionView registerNib:[UINib nibWithNibName:CARD_HEADER_ITEM
                                                    bundle:nil]
          forCellWithReuseIdentifier:CARD_HEADER_ITEM];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (self.typeView != TYPE_FULL_VIEW) {
        ...
        switch (index) {
            case 0: {
                UICollectionViewCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:CARD_HEADER_ITEM
            ...
            case 1: {
                return [collectionView dequeueReusableCellWithReuseIdentifier:CARD_TITLE_ITEM
                                                                 forIndexPath:indexPath];
            }
        }
    }
    ...
    UILabel * title = (UILabel *)[cell viewWithTag:1010];
    title.text = [NSString stringWithFormat:@"This is element #%ld", indexPath.row-1+self.typeView];
    return cell;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row >= index) {
        ...
        if (view != nil) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:EVENT_VIDEO_ITEM_CLICK
                                                                object:nil
